...
class Game2048:
    def __init__(self):
        ...
        self.score = 0
        self.moved = False
        self.merged_tiles = 0  # Track number of merges in last move
        self.spawned_new_tile = False  # Track if new tile was spawned
        self._spawn_tile()
        self._spawn_tile()
    ...
    def _slide_line(self, line: np.ndarray) -> tuple[np.ndarray, int, int]:
        line = line[line != 0]
        score_gained = 0
        merges = 0
        ...
        return result, score_gained, merges
    ...
    def move_left(self) -> bool:
        ...
        for i in range(4):
            ...
            self.grid[i], score, merges = self._slide_line(self.grid[i])
            total_score += score
            self.merged_tiles += merges
    ...
    def move(self, direction: str) -> bool:
        ...
        # Reset spawn flag
        self.spawned_new_tile = False

        if moves[direction]():
            self._spawn_tile()
            self.spawned_new_tile = True
            return True
        return False
