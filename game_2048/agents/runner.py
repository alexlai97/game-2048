...
class AIRunner:
    ...
    def run_single_game(self, agent: BaseAgent, max_moves: int = 10000, 
                       game_callback: Optional[Callable] = None) -> GameResult:
        """
        ...
        if self.verbose:
            ...
            print(f"Score: {result.score}, Moves: {result.moves}, Max tile: {result.max_tile}")
            print(f"Duration: {duration:.2f}s")
        
        return result
    
    def run_batch(self, agent: BaseAgent, num_games: int, 
                  max_moves: int = 10000) -> List[GameResult]:
        """
    ...
    def save_results(self, results: List[GameResult], agent: BaseAgent, 
